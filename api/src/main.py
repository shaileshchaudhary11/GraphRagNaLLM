...
from llm.ollamaai import OllamaLlama3
...
neo4j_connection = Neo4jDatabase(
    host=os.environ.get("NEO4J_URL", "neo4j://74.225.197.5:60001"),
...
async def questionProposalsForCurrentDb(payload: questionProposalPayload):
    ...
    questionProposalGenerator = QuestionProposalGenerator(
        ...
        llm=OllamaLlama3(api_endpoint=api_endpoint),
    )
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            default_llm = OllamaLlama3(api_endpoint=api_endpoint)
...
async def companyInformation(payload: companyReportPayload):
    ...
    llm = OllamaLlama3(api_endpoint=api_key)
